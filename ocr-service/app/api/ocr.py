...
from app.services.ocr_engine import ocr_engine, OCR_POOL_SIZE
...
def process_single_image(
    ...
    """
    Runs the full per-image pipeline (download -> quality -> preprocess
    -> OCR) for one image. Pulled out into its own function so multiple
    images can be run concurrently via a thread pool instead of one
    after another.
    """
...
def extract_text(
    ...
    try:
        ...
        with ThreadPoolExecutor(
            ...
            image_results = list(
                executor.map(
                    lambda item: process_single_image(
                        item[0], item[1]
                    ),
                    enumerate(
                        request.image_urls, start=1
                    ),
                )
            )
